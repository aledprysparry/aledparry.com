...
import { getBusinessBySlug, getLocationBySlug, getMenu, listActiveOffers } from "@/lib/buan/api";
...
// Demo end-of-day offer (clearing remaining stock): 30% off the Latte + Bacon Roll.
const SAMPLE_OFFERS: Offer[] = [
  { id: "o1", productIds: ["s2", "s3"], discountPct: 30, active: true, label: "End-of-day" },
];
...
export default async function LocationMenu({
  ...
  const offers = await listActiveOffers(location.id);

  return <OrderFlow live business={business} location={location} menu={menu} offers={offers} />;
}
