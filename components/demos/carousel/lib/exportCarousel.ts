...
import { CanvasRenderer } from '@carousel/canvas/CanvasRenderer';
...
async function renderSlideBlob(
  ...
  const r = new CanvasRenderer(canvas, 'portrait');
...
export async function exportSlide(
  ...
  const blob = await renderSlideBlob(slide, index, rows, copy, slideCount, mime);
...
export async function exportZip(
  ...
  for (let i = 0; i < slides.length; i++) {
    const blob = await renderSlideBlob(slides[i], i, rows, copy, slides.length, mime);
