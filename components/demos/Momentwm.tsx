  ...
  return (
    <div className="mw-creu-back" onClick={onClose}>
      <div className="mw-creu" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mw-cr-note">{d?.templated === false ? "Drafft golygyddol (Claude). Adolygwch cyn ei gyhoeddi." : "Cymraeg drafft (templed) yw hwn."} Ffynhonnell: <a href={opp.candidate.sourceUrl} target="_blank" rel="noreferrer">{opp.candidate.sourceName} &#8599;</a> &middot; Cefndir: <a href={`https://www.britannica.com/search?query=${encodeURIComponent(opp.candidate.eventTitle)}`} target="_blank" rel="noreferrer" title="Chwilio am y pwnc ar Britannica">Britannica &#8599;</a></p>
      </div>
    </div>
  );
}
