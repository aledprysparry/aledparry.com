...
const announceSlide: SlideDef = {
  ...
  draw(r, props) {
    ...
    // Hook - huge, bold, left-aligned, gold Digitalt. The whole point.
    setShadow(r, 'rgba(22,9,66,0.5)', 0.005, 0.009, 0.002);
    r.drawTextWrapped(c.hook || 'Barod i chwarae?', { x: L, y: 0.21, size: 0.115, color: GOLD, weight: '900', align: 'left', font: DISPLAY, maxWidth: 0.5, lineHeight: 0.118, baseline: 'top' });
    clearShadow(r);

    // Stacked sub lines (the third one gold for a lift).
    setShadow(r, 'rgba(8,3,34,0.4)', 0, 0.005, 0.0012);
    r.drawText(c.sub1 || 'Wythnos newydd', { x: L, y: 0.475, size: 0.045, color: WHITE, weight: '800', align: 'left', baseline: 'top', font: BODY });
    r.drawText(c.sub2 || 'Sgorfwrdd gwag', { x: L, y: 0.535, size: 0.045, color: WHITE, weight: '800', align: 'left', baseline: 'top', font: BODY });
    r.drawText(c.sub3 || 'Pob lwc!', { x: L, y: 0.595, size: 0.045, color: GOLD, weight: '900', align: 'left', baseline: 'top', font: BODY });
    clearShadow(r);

    // Warm welcome, smaller, left column.
    r.drawTextWrapped(c.welcome || 'Croeso cynnes i bawb sy’n ymuno am y tro cyntaf yr wythnos hon.', { x: L, y: 0.685, size: 0.028, color: 'rgba(255,255,255,0.86)', weight: '600', align: 'left', font: BODY, maxWidth: 0.46, lineHeight: 0.038, baseline: 'top' });
    ...
    // The phone, stood up on the RIGHT (the app as proof). Defaults to the
    // scoreboard; blank-phone placeholder when no image is set.
    const bx = 0.55, bw = 0.44, by = 0.17, bh = 0.66;
