            ...
            <label key={f.key} className="flex flex-col gap-1">
              ...
              {f.multiline ? (
                <textarea
                  ...
                  rows={3}
                  className="resize-y rounded-lg border border-zinc-200 bg-white px-3 py-2 text-[13px] leading-snug text-zinc-800 focus:border-violet-400 focus:outline-none focus-visible:ring-1 focus-visible:ring-violet-500 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                />
              ) : (
                <input
                  value={copy[f.key] ?? ''}
