...
function generatePremiereScript(graphics, ratio, prefix, projectName) {
  ...
  return `// RENAME THIS FILE: Change .txt to .jsx before running in Premiere
// Premiere Pro ExtendScript — Auto-import PNG sequences + place on timeline
// Generated by Social Editor for: ${(projectName||"Project").replace(/"/g,"'")}
// Ratio: ${ratio} (${W}x${H}) @ ${FPS}fps
//
// HOW TO USE:
// 1. Unzip the export folder
// 2. In Premiere: File > Scripts > Run Script
// 3. Select this .jsx file
// 4. Choose the unzipped folder when prompted
// 5. Done — sequences placed on V2 (fullscreen) and V3 (overlays)
...
function main() {
  ...
  var folder = Folder.selectDialog("Select the unzipped export folder");
  if (!folder) return;

  var rootItem = proj.rootItem;

  // Create bins
  var fsBin = null, ovBin = null;
  for (var b = 0; b < rootItem.children.numItems; b++) {
    if (rootItem.children[b].name === "Fullscreen Graphics") fsBin = rootItem.children[b];
    if (rootItem.children[b].name === "Overlay Graphics") ovBin = rootItem.children[b];
  }
  if (!fsBin) { fsBin = rootItem.createBin("Fullscreen Graphics"); }
  if (!ovBin) { ovBin = rootItem.createBin("Overlay Graphics"); }

  // Create sequence if none active
  var seq = proj.activeSequence;
  if (!seq) {
    proj.createNewSequence("${(projectName||"Graphics").replace(/"/g,"'")}", "seqPreset");
  ...
  for (var i = 0; i < clips.length; i++) {
    var c = clips[i];
    var seqFolder = new Folder(folder.fsName + "/" + c.folder);

    if (!seqFolder.exists) {
      // Try importing still PNG instead
      var stillFile = new File(folder.fsName + "/" + c.folder + ".png");
      if (stillFile.exists) {
        var ok = proj.importFiles([stillFile.fsName], false, rootItem, false);
        if (ok) {
          var item = findItemByName(rootItem, c.folder + ".png");
          if (item) {
            var bin = c.isOv ? ovBin : fsBin;
            item.moveBin(bin);
            var track = c.isOv ? 2 : 1; // V3 for overlays, V2 for fullscreen
            var startTicks = c.startSec * 254016000000; // Premiere ticks per second
            seq.videoTracks[track].insertClip(item, startTicks / 254016000000);
            imported.push(c.label + " (still)");
          }
        }
      }
      continue;
    ...
    // Get first frame to import as sequence
    var files = seqFolder.getFiles("*.png");
    if (files.length === 0) continue;
    files.sort();

    // Import as image sequence
    var firstFrame = files[0];
    if (proj.importFiles([firstFrame.fsName], true, rootItem, true)) {
      // Find the imported item
      var item = findLastImported(rootItem);
      if (item) {
        var bin = c.isOv ? ovBin : fsBin;
        item.moveBin(bin);

        // Place on timeline
        var track = c.isOv ? 2 : 1;
        var startTime = c.startSec;
        seq.videoTracks[track].insertClip(item, startTime);
        imported.push(c.label);
      }
    }
  }

  alert("Imported " + imported.length + " of " + clips.length + " graphics:\\n\\n" + imported.join("\\n"));
}
...
function findLastImported(root) {
...
function ExportTab({project,brand,updateProject}){
  ...
  const exportCueSheet=async()=>{
    ...
    const hdrShade={fill:"1B4965",type:ShadingType.CLEAR};
    const hdrRun=(t)=>new TextRun({text:t,bold:true,color:"FFFFFF",font:"Arial",size:18});
    const cellRun=(t,opts={})=>new TextRun({text:t,font:"Arial",size:18,...opts});

    // Column widths (DXA) — total 9360 (US Letter with 1" margins)
    const colW=[600,1400,900,1600,1200,3660];

    const headerRow=new TableRow({children:["#","Timecode","Duration","Template","Type","Content"].map((h,i)=>
      new TableCell({borders,width:{size:colW[i],type:WidthType.DXA},shading:hdrShade,margins:cellMar,
        children:[new Paragraph({alignment:AlignmentType.LEFT,children:[hdrRun(h)]})]})
    )});
    ...
    const dataRows=gfx.map((g,i)=>{
      ...
      const altShade=i%2===0?{fill:"F5F0EB",type:ShadingType.CLEAR}:undefined;
      return new TableRow({children:vals.map((v,ci)=>
        new TableCell({borders,width:{size:colW[ci],type:WidthType.DXA},shading:altShade,margins:cellMar,
          children:[new Paragraph({children:[cellRun(v)]})]})
      )});
    });
    ...
    const doc=new Document({
      ...
      sections:[{
        ...
        children:[
          new Paragraph({heading:HeadingLevel.HEADING_1,spacing:{after:120},children:[new TextRun({text:project.name||"Graphics Cue Sheet",bold:true,font:"Arial",size:36,color:"1B4965"})]}),
          new Paragraph({spacing:{after:300},children:[cellRun(`Exported ${new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}  •  ${gfx.length} graphics`,{color:"666666",size:18})]}),
          new Table({width:{size:9360,type:WidthType.DXA},columnWidths:colW,rows:[headerRow,...dataRows]}),
          new Paragraph({spacing:{before:400},children:[cellRun(`Generated by Social Editor  •  aledparry.com`,{color:"999999",size:16,italics:true})]}),
        ]
      }]
    });
  ...
  const runExport=async()=>{
    ...
    for(const ratio of selectedRatios){
      ...
      if(gfxMode==="png"||gfxMode==="both"){
        ...
        dlText(script,`${pn}_${prefix}PREMIERE_IMPORT_SCRIPT.txt`);
      }
      ...
      if(gfxMode==="premiere"||gfxMode==="pngseq"){
        ...
        const script=generatePremiereScript(selectedGfx,ratio,prefix,project.name);
        zip.file(`${pn}_${prefix}PREMIERE_IMPORT_SCRIPT.txt`,script);
